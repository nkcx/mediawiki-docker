...
3. **Volume init** — on first run or a MediaWiki version change, replaces the bundled extensions/skins on the `/extensions` and `/skins` volumes with the image's copies (git-managed ones are kept), then symlinks those volumes back into the webroot. The image records its bundled lists in `/usr/local/share/mediawiki-bundled-{extensions,skins}`.
...
5. **Composer** — generates `composer.local.json` from `MW_COMPOSER_PACKAGES` and merges the `composer.json` of bundled and git-managed extensions (never Composer-installed ones). Runs `composer update` on every start, as on a normal install. If it fails but every requested package is already in `vendor/composer/installed.json`, startup continues with the installed versions. `/composer-cache`, if mounted, is used as the download cache. `vendor/` can be a volume mounted directly at `/var/www/html/vendor` (never symlinked: the autoloader derives paths from its real location); `init_vendor` restores core's copy from `/usr/local/share/mediawiki-vendor` whenever `/usr/local/share/mediawiki-image-fingerprint` differs from the one recorded in the volume.
5b. Git-managed extensions and skins are processed *before* Composer, so their own `composer.json` files are on disk when the merge list is built.
