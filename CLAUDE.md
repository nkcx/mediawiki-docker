...
5. **Composer** — generates `composer.local.json` from `MW_COMPOSER_PACKAGES` and merges the `composer.json` of bundled and git-managed extensions (never Composer-installed ones). If a fingerprint of the image, the package list and the merged files matches `/config/composer.fingerprint`, runs `composer install` from `/config/composer.lock`; otherwise runs `composer update` and saves the new lock and fingerprint. `/composer-cache`, if mounted, is used as the download cache. `vendor/` itself stays in the container, so an image upgrade always starts from the image's own core libraries.
