...
5. **Composer** — generates `composer.local.json` from `MW_COMPOSER_PACKAGES` and merges the `composer.json` of bundled and git-managed extensions (never Composer-installed ones). Runs `composer update` on every start so floating constraints pick up new releases, and saves the result to `/config/composer.lock` with a fingerprint of the image, package list and merged files. If the update fails and the fingerprint is unchanged, reinstalls the saved lock instead. `/composer-cache`, if mounted, is used as the download cache. `vendor/` itself stays in the container, so an image upgrade always starts from the image's own core libraries.
